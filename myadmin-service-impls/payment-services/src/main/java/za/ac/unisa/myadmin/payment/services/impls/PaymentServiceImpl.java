package za.ac.unisa.myadmin.payment.services.impls;
...
public class PaymentServiceImpl implements PaymentService {
	...
	public CreditCardPaymentInfo processStudentInput(Integer userId) throws OperationFailedException {
		try {
			...
			ccPaymentsProxy.setInCsfClientServerCommunicationsClientVersionNumber(
					CommonServicesConstants.PROXY_CLIENT_SERVER_COMMUNICATIONS_CLIENT_VERSION);
			ccPaymentsProxy.setInCsfClientServerCommunicationsClientRevisionNumber(
					CommonServicesConstants.PROXY_CLIENT_SERVER_COMMUNICATIONS_CLIENT_REVISION);
			ccPaymentsProxy.setInCsfClientServerCommunicationsAction(
					PaymentServicesConstants.CREDIT_CARD_PAYMENT_STUDENT_PROXY_CLIENT_SERVER_COMMUNICATIONS_ACTION);
			ccPaymentsProxy.setInCsfClientServerCommunicationsClientDevelopmentPhase(
					CommonServicesConstants.PROXY_CLIENT_SERVER_COMMUNICATIONS_CLIENT_DEVELOPMENT_PHASE);
			ccPaymentsProxy.setInSecurityWsUserNumber(PaymentServicesConstants.CREDIT_CARD_PAYMENT_PROXY_USER_NUMBER);
			ccPaymentsProxy.setInWsWorkstationCode(CommonServicesConstants.PROXY_WORKSTATION_CODE);
	...
	public QualPaymentInfo processQualInput(Integer studentNumber, String qualCode) throws OperationFailedException {
		...
		try {
			...
			ccPaymentsProxy.setInCsfClientServerCommunicationsClientRevisionNumber(
					CommonServicesConstants.PROXY_CLIENT_SERVER_COMMUNICATIONS_CLIENT_REVISION);
			ccPaymentsProxy.setInCsfClientServerCommunicationsAction(
					PaymentServicesConstants.CREDIT_CARD_PAYMENT_QUAL_PROXY_CLIENT_SERVER_COMMUNICATIONS_ACTION);
			ccPaymentsProxy.setInCsfClientServerCommunicationsClientDevelopmentPhase(
					CommonServicesConstants.PROXY_CLIENT_SERVER_COMMUNICATIONS_CLIENT_DEVELOPMENT_PHASE);
			ccPaymentsProxy.setInSecurityWsUserNumber(PaymentServicesConstants.CREDIT_CARD_PAYMENT_PROXY_USER_NUMBER);
			ccPaymentsProxy.setInWsWorkstationCode(CommonServicesConstants.PROXY_WORKSTATION_CODE);
...
//				eventTrackingService.newEvent(EventTrackingTypes.EVENT_CREDITCARD_PAYMENT, toolManager.getCurrentPlacement().getContext(), false));
	...
	private Sfrrf03sMntOnlineCcPayments getPaymentProxyInstance() throws PropertyVetoException {
		...
		ccPaymentsProxy.setInCsfClientServerCommunicationsClientRevisionNumber(
				CommonServicesConstants.PROXY_CLIENT_SERVER_COMMUNICATIONS_CLIENT_REVISION);
		ccPaymentsProxy.setInCsfClientServerCommunicationsAction(
				PaymentServicesConstants.CREDIT_CARD_PAYMENT_PAYMENT_PROXY_CLIENT_SERVER_COMMUNICATIONS_ACTION);
		ccPaymentsProxy.setInCsfClientServerCommunicationsClientDevelopmentPhase(
				CommonServicesConstants.PROXY_CLIENT_SERVER_COMMUNICATIONS_CLIENT_DEVELOPMENT_PHASE);
		ccPaymentsProxy.setInSecurityWsUserNumber(PaymentServicesConstants.CREDIT_CARD_PAYMENT_PROXY_USER_NUMBER);
