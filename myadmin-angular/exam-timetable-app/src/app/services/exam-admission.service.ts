...
export class ExamAdmissionService {
  ...
  getExamAdmissions(): Observable<ExamAdmissionInfo[]> {
    return this.http.get<ExamAdmissionInfo[]>(`${environment.exam_admission_service_host_url}/rest/examadmissions`);
  }

  getExamAdmissionsByYear(year: number): Observable<ExamAdmissionInfo[]> {
    const params = new HttpParams().set('year', <string><any>year);

    return this.http.get<ExamAdmissionInfo[]>(`${environment.exam_admission_service_host_url}/rest/examadmissions`, { params });
  }
